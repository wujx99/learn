...
按 v1 → v2 → v3 阅读最清楚：v1 回答“如何围绕稀疏 3D anchor 从多视角、多尺度、多帧图像取证”；v2 把随历史长度线性增长的多帧采样改成递归实例记忆；v3 保留 v2 的推理结构，重点修复 one-to-one 训练、置信度排序和 query attention，并利用 query 的跨帧连续性直接分配跟踪 ID。
